...
// $Author:     MATEJ SLIVKA <xslivk03@stud.fit.vutbr.cz>
// $Date:       $2021-02-27
//============================================================================//
/**
...
TEST_F(EmptyTree, InsertNode)
{
	...
	EXPECT_TRUE(koren == NULL); // predpoklad ze strom je prazdny teda ze nema koren
	auto prirad = tree.InsertNode(1);
	EXPECT_TRUE(prirad.first);
	EXPECT_TRUE(prirad.second->key == 1); //overenie ze ci program pridal prvok do stromu
	prirad = tree.InsertNode(1);
	EXPECT_FALSE(prirad.first); // druhy pokus o vklad toho isteho prvku
}
...
{
	std::vector<BinaryTree::Node_t*> outAllNodes; // vytvorenie pola na ukladanie prvkov stromu
	tree.GetAllNodes(outAllNodes);
	int velkost1 = sizeof(outAllNodes);
	...
	tree.GetAllNodes(outAllNodes);
	int velkost2 = sizeof(outAllNodes);

	EXPECT_TRUE(velkost2>velkost1);
...
TEST_F(TreeAxioms, Axiom2)
{
	...
	for (int i = 0; i < outAllNodes.size() - 1; i++)
	{
		...
		{
			outAllNodes[i]->pLeft->color == BLACK;
			outAllNodes[i]->pRight->color == BLACK;
		}
	}
}
